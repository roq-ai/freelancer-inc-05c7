...
import { useContractFindFirst, useRoqClient } from 'lib/roq';
...
function ContractViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; project: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useContractFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('contract', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/contracts/edit/${id}`} passHref legacyBehavior>
                    <Button
