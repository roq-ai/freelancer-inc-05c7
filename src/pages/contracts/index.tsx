...
import { useRoqClient, useContractFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ContractInterface, unknown>;
...
export function ContractListPage(props: ContractListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useContractFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.contract.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ContractInterface) => {
    if (hasAccess('contract', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="contract" />
        </Flex>

        {hasAccess('contract', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/contracts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
